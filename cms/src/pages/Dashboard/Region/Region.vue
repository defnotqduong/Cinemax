    ...
    <div class="mt-[8px] mb-24 pr-6 pl-8">
        <div class="text-sm breadcrumbs">
            <ul class="justify-start">
                ...
                <li><router-link :to="{ name: 'dashboard-movie' }" class="text-primary">Thể loại</router-link></li>
                <li>Danh sách</li>
            </ul>
        </div>
...
export default defineComponent({
    ...
    setup() {
        ...
        return { loading, overview, meta, links, filter, categories }
    },
    methods: {
        async getData(page) {
            this.loading = true
            ...
            const data = await getAllCategory(this.filter)
            ...
            if (data && data.success) {
                ...
                this.overview.from = data.categories.from
                this.overview.to = data.categories.to
                this.overview.total = data.categories.total

                this.meta.current_page = data.categories.current_page
                this.meta.last_page = data.categories.last_page

                this.links.first_page_url = data.categories.first_page_url
                this.links.last_page_url = data.categories.last_page_url
                this.links.prev_page_url = data.categories.prev_page_url
                this.links.next_page_url = data.categories.next_page_url
            }

    ...
    <div class="mt-[8px] mb-24 pr-6 pl-8">
        ...
        <div class="my-2 flex items-start justify-between">
            <div class="flex items-end justify-start gap-4">
                <h3 class="text-4xl font-bold">Khu vực</h3>
                <p v-if="overview.total > 0" class="text-[15px] mb-[2px]">Hiển thị từ {{ overview.from }} đến {{ overview.to }} trong tổng số {{ overview.total }} khu vực</p>
                <p v-if="overview.total == 0" class="text-[15px] mb-[2px]">Không có bản ghi nào</p>
            </div>
        </div>
        <div class="mb-3 flex items-center justify-between">
            <router-link :to="{ name: 'dashboard-region-create' }" class="btn px-2 h-10 min-h-10 gap-1 text-sm text-white bg-green hover:bg-green hover:opacity-80">
        ...
        <div v-if="!loading">
            <div class="overflow-x-auto rounded-md mb-6 bg-white border-[1px] border-gray-300">
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="region in regions" :key="region.id" class="hover:bg-[rgba(124,105,239,0.1)] border-none">
                            <td class="py-0">
                                {{ region.name }}
                            </td>
                            <td class="py-0">
                            ...
                            <td class="py-0">
                                <div class="flex items-center justify-start gap-2">
                                    <router-link
                                        :to="{ name: 'dashboard-region-edit', params: { id: region.id } }"
                                    ...
                                    <div>
                                        ...
                                        >
                                            ...
                                            >Xóa</label
                                        >
                                        <input type="checkbox" :id="'my_modal_' + region.id" class="modal-toggle" />
                                        <div class="modal" role="dialog">
                                            <div class="modal-box bg-white p-8 flex flex-col gap-8">
                                                <div class="flex flex-col items-start justify-start">
                                                    ...
                                                    <p class="text-base">
                                                        ...
                                                        <span class="text-red-600 font-extrabold">{{ region.name }}</span> không?
                                                    </p>
                                                </div>
                                                <div class="flex items-center justify-end gap-4">
                                                    <div class="modal-action">
                                                        <label @click="deleted(region.id)" :for="'my_modal_' + region.id" class="btn px-3 h-10 min-h-10 bg-red-500 text-white hover:bg-red-700">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" viewBox="-3 0 32 32">
                                                                <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                                                    <g id="Icon-Set-Filled" transform="translate(-261.000000, -205.000000)" fill="currentColor">
                                                                        <path
                                                    ...
                                                    <div class="modal-action">
                                                        <label :for="'my_modal_' + region.id" class="btn px-3 h-10 min-h-10 bg-gray-400 text-white hover:bg-gray-500">
                                            ...
                                            <label class="modal-backdrop" :for="'my_modal_' + region.id">Close</label>
                                        </div>
                                    </div>
                                </div>
                            </td>
...
import { getAllRegion, deleteRegion } from '../../../webServices/regionService'
export default defineComponent({
    ...
    setup() {
        ...
        const getData = async page => {
            ...
            const data = await getAllRegion(filter.value)

            if (data && data.success) {
                regions.value = data.regions.data

                overview.value.from = data.regions.from
                overview.value.to = data.regions.to
                overview.value.total = data.regions.total

                meta.value.current_page = data.regions.current_page
                meta.value.last_page = data.regions.last_page

                links.value.first_page_url = data.regions.first_page_url
                links.value.last_page_url = data.regions.last_page_url
                links.value.prev_page_url = data.regions.prev_page_url
                links.value.next_page_url = data.regions.next_page_url
            }
        ...
        return { loading, overview, meta, links, filter, regions, getData, deleted }
    },
    methods: {},
    created() {
        this.getData()

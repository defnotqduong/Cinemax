            ...
            <div v-if="!loading" class="w-[75%] mt-5 text-[15px]">
                <div class="p-6 bg-white border-[1px] border-gray-400 rounded-lg">
                    <div class="grid grid-cols-1 gap-x-6 gap-y-4">
                        <label for="name">
                            <div class="mb-2 text-gray-700 font-bold" :class="errors.hasOwnProperty('name') ? 'text-red-500' : 'text-gray-700'">Tên thể loại <span class="text-red-500">*</span></div>
                            <input
                                type="text"
                                id="name"
                                v-model="name"
...
import { getCategory, editCategory } from '../../../webServices/categoryService'
export default defineComponent({
    setup() {
        ...
        const edit = async e => {
            ...
            const data = await editCategory(category.id, category)
    ...
    methods: {
        async getData() {
            ...
            const [category] = await Promise.all([getCategory(id)])

            if (category && category.status === 404) this.$router.push({ name: 'dashboard-category' })

            if (category && category.success) {
                this.id = category.category.id
                this.name = category.category.name
                this.des = category.category.des
            }
